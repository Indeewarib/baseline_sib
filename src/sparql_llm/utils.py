...
def query_sparql(
    ...
    If use_file is provided, it will be used as a fallback when the endpoint fails or returns no results.
    """
    ...
    try:
        ...
        try:
            ...
            if result_count > 0:
                ...
                logger.info(f"ℹ️ Endpoint returned 0 results, will try fallback file")
        except Exception as e:
            logger.warning(f"⚠️ Endpoint error: {e}")
        
        # Fallback: use file if endpoint failed or returned no results
        if use_file:
            logger.info(f"📖 Using fallback file: {use_file}")
            try:
                g = rdflib.Graph()
                ...
                result_count = len(query_resp.get("results", {}).get("bindings", []))
                logger.info(f"✅ Loaded {result_count} results from fallback file")
            except Exception as e:
                logger.warning(f"⚠️ Error reading fallback file: {e}")
        else:
            logger.warning(f"⚠️ No fallback file provided for {endpoint_url}")
    finally:
